        ...
        if click=='enviar':
            ...
            for i in range(0, rangomcod):
                if ldnbin[i]==mbin[i]:
                    ...
                    mcod=mcod+"1"
            #---------------------------------------------------------
            #MENSAJE ENCRIPTADO A ENVIAR
            mensaje_a_enviar=ca+mcod
            mensaje_encriptado=mensaje_a_enviar
            
            #ENVIO DE MENSAJE

            mredactado=str(whatsintro_gui.mensaje_redactado())

            if mredactado!="":
                enviar_mensaje=whatsintro_msg.enviar_mensaje(mensaje_encriptado)
                enviar_mensaje
                if enviar_mensaje==True:
                    whatsintro_gui.borrar_mensaje_redactado()
                elif enviar_mensaje==False:
                    msg="ERROR"
