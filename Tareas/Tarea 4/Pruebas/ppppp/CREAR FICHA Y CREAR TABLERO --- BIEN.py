...
def cargar_tablero_con_fichas():
    ...
    filasT=(tablero_cargado.filasT())
    columnasT=(tablero_cargado.columnasT())
    ...
    while proxima_ficha_o_terminar!="0":
        ...
        if proxima_ficha_o_terminar!="0":
            ...
            if posicion_valida(filasT, columnasT, posicion_inicial)==True:
                ficha_cargada.posicionar_en(posicion_inicial)
                posiciones=ficha_cargada.posicion().split(",")
                posicionF=posiciones[0]
                posicionC=posiciones[1]
            ...
            break
    return(tablero_cargado.fichas_en_tablero())
def imprimir_tablero_con_fichas(tablero_con_fichas):
    tablero_con_fichas.mostrar_tablero()

def filasT(lista_tablero_txt):
    dimensiones=lista_tablero_txt[0].split(",")
    filasT=int(dimensiones[0])
    return filasT
def columnasT(lista_tablero_txt):
    dimensiones=lista_tablero_txt[0].split(",")
    columnasT=int(dimensiones[1])
    return columnasT
...
class Tablero():
    fichas_en_tablero=[]
    lista_con_fichas=["O: Posición libre | ","X: Posición prohibida | "] #Fichas con sus simbolos respectivos
    def __init__(self,nombre_archivo):
        self.nombre_tablero=str(nombre_archivo)
        tablero_txt=open(str(nombre_archivo),"r+")
        self.lista_tablero_txt=tablero_txt.readlines()
        tablero_txt.close()
        print(self.lista_tablero_txt)
        if len(self.lista_tablero_txt)>1:
            self.posiciones_prohibidas=self.lista_tablero_txt[1:]
        self.filas=int(filasT(self.lista_tablero_txt))
        self.columnas=int(columnasT(self.lista_tablero_txt))
        ...
        self.filasT=self.filas
        self.columnasT=self.columnas
    ...
    def agregar_ficha(self, nombre_ficha, posicionF, posicionC):
        ...
        self.lista_con_fichas.append(agregar)
        self.fichas_en_tablero[int(posicionF)][int(posicionC)]=self.simbolo_ficha()
...
while True:
    ...
    if verificar_numero_0a3(opcion)==True:
        opcion=int(opcion)
        if opcion==3:
            tablero_cargado_con_fichas=cargar_tablero_con_fichas()
            mostrar_menu_cargar()
            opcionC=(input("Seleccione una opción: "))
            if opcionC==1:
                imprimir_tablero_con_fichas(tablero_cargado_con_fichas)
            if opcionC==2:
                pass
            if opcionC==3:
                pass
