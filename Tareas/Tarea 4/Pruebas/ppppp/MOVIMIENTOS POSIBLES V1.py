...
def verificar_numero_0a3(n):
    ...
    for c in str(n):
        if ord(c)>47 and ord(c)<52:
            ...
            return False
def verificar_numero_0aX(n,X):
    for c in str(n):
        if ord(c)>ord("0") and ord(c)<ord(str(X)):
            return True
        else:
            return False
...
def mostrar_posibles_movimientos_ficha(tablero):
    ...
    numero_ficha=-1
    numero_ficha=(input("Seleccione una ficha: "))
    while verificar_numero_0aX(numero_ficha,tablero.fichas_posicionadas[len(tablero.fichas_posicionadas)-1][0])==False or int(numero_ficha)>len(tablero.fichas_posicionadas) or int(numero_ficha)<=0:
