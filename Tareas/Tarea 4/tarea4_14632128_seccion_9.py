...
def mostrar_posibles_movimientos_ficha(tablero):
    ...
    numero_ficha=str(input("Seleccione una ficha: "))
    cont=False
    while cont==False:
        if len(str(numero_ficha))==1:
            if ord(str(numero_ficha))>ord("0") and ord(str(numero_ficha))<=ord(str(len(tablero.fichas_posicionadas))):
                cont=True
                break
            else:
...
class Tablero():
    ...
    def se_puede_recorrer(self, archivo_salida):
        ...
        print("")
        print("Esto puede demorar bastante...")
        print("")
        se_puede=False
        a=datetime.datetime.now()
        #print(a)
        while se_puede==False:
            LLL=posibles_movimientos_ficha_recursion(tablero_cargado)
            #agregar el movimiento 0,0 a todas las fichas
            for i in range(len(LLL)):
                LLL[i][4].append("0,0")
            #guardar posiciones originales:
            posiciones_originales=[]
            for i in range(len(LLL)):
                posiciones_originales.append("("+str(LLL[i][2])+","+str(LLL[i][3])+")")
            L=copy.deepcopy(LLL)
            tablero2=copy.deepcopy(self.casillas_tablero)
            nuevas_posiciones_fichas=[]
            for i in range(len(L)):
                nuevas_posiciones=[]
                for k in range(len(L[i][4])):
                    mov=(random.choice(L[i][4]))
                    mov=mov.split(",")
                    ...
                    if nF>=0 and nC>=0 and nF<self.F and nC<self.C and tablero2[nF][nC]=="O":
                        ...
                        L[i][2]=nF
                        L[i][3]=nC
                        nuevas_posiciones.append(("("+str(nF)+", "+str(nC)+")"))
                nuevas_posiciones_fichas.append(nuevas_posiciones)
            ...
            t=(str(c-a))[3:7]                     
            if t=="4:59":
                print("Tiempo máximo para 'verificar si se pueden mover las piezas sobre el tablero sin repetir posiciones recorrer' excedido...")
                print("Intente con otra distribución de fichas...")
                break
            ...
            if contador_vacios==0:
                ...
                c=datetime.datetime.now()
                t=(str(c-a))[3:7]   
                print("Resultado exitoso obtenido en aproximadamente "+t+" (minutos:segundos).")
                print("Puede revisar el resultado de la verificación en el archivo creado con el nombre: "+str(archivo_salida))
                cadena_fichas=""
                for i in range(len(L)):
                    if i==0:
                        cadena_fichas+=str(L[0][1])
                    elif i==len(L)-1 and len(L)!=1:
                        cadena_fichas+=" y "+str(L[i][1])
                    elif i==len(L)-1 and len(L)==1:
                        cadena_fichas+=str(L[i][1])
                    else:
                        cadena_fichas+=","+str(L[i][1])+" "
                output=open(str(archivo_salida),"w")
                output.write("Recorriendo el tablero "+str(self.nombre_tablero)+" con la(s) ficha(s) "+cadena_fichas+"\n")
                for i in range(len(L)):
                    output.write(str(L[i][1])+": "+str(posiciones_originales[i])+" -> ")
                    for j in range(len(nuevas_posiciones_fichas[i])):
                        if j==len(nuevas_posiciones_fichas[i])-1:
                            output.write(str(nuevas_posiciones_fichas[i][j]))
                        else:
                            output.write(str(nuevas_posiciones_fichas[i][j])+" -> ")
                    output.write("\n")
...
###
'''
Alumno: Javier Lopez Achondo
N Alumno: 14632128
Seccion: 9
Profesor: Jesus Bellido
'''
###
